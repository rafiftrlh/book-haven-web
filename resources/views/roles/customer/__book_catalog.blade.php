...
<div class="container-fluid p-4">
    ...
    <div style="overflow-x: auto; overflow-y: hidden; display: flex; gap: 5px;">
        @foreach ($categories as $category)
            <button type="button" class="btn btnCategory btn-secondary category-filter" style="text-transform: capitalize;" data-category-id="{{ $category->id }}">
                {{ $category->name }}
            </button>
        @endforeach
    </div>
        ...
        @foreach ($books as $book)
            <div class="card p-3" style="height: fit-content; width: 230px;">
                ...
                <div class="card-body px-0 pt-3 pb-0">
                    ...
                    <p style="text-transform: capitalize; overflow: hidden; display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 2;" class="text-success mb-2 text-sm">
                    ...
                    <p style="text-transform: capitalize; overflow: hidden; display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 1; font-size: 12px; font-weight: 600" class="text-muted">
                        By :
                    ...
                    <button type="button" class="text-info icon-move-right pull-right no-style-button fw-bold" style="font-size: 14px" data-bs-toggle="modal" data-bs-target="#detailBook_{{ $book->id }}" data-book-id="{{ $book->id }}">
                        Read More <i class="fas fa-arrow-right text-sm ms-1" aria-hidden="true"></i>
                    </button>
                    @include('partials.modals.customer.__detail_book')
                </div>
                <div class="card-footer d-flex justify-content-between p-0 mt-3 pt-4" style="border-top: 2px solid rgba(128, 128, 128, 0.21)">
                    <button type="button" class="btn @if ($book->stock == 0) btn-secondary @else bg-gradient-primary @endif" @if ($book->stock == 0) @disabled(true) @endif onclick="confirmBorrow({{ $book->id }})">Borrow</button>
